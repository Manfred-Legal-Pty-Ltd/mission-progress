window.DASHBOARD_DATA = {
  ...
  kpis: [
    ...
    { key: "blockers", label: "Blockers", value: "0", subLabel: "None open", detail: "The release dependency that was open has cleared: the company-side GitHub billing item is resolved. v0.1.37 is the current released version, verified on the actual released build. It reaches the firm through the usual automatic update: Gideon detects it, downloads it in the background, and prompts a restart to move to the new version." },
  ...
  remarks: [
    { date: "18 September 2026", text: "Gideon v0.1.37 is released and reaches the firm through the usual automatic update, with nothing to install by hand. It carries two improvements: a natural-language document search now reliably finds matter documents it could previously miss, and a document dropped straight into the matter conversation opens the existing check-before-importing review for that matter, with nothing imported until you confirm. Both were verified on the actual released build after the automatic update moved the installed app to v0.1.37. The conversational command layer (Milestone 2) has passed engineering validation for recognising what you mean within the matter you have open, or asking rather than guessing when a reference is unclear; the remaining acceptance step is the firm's confirmation that this meets expectations in practice, so Milestone 2 stays in progress and is not yet marked passed. Milestone 3 continues in parallel and the programme is on track. Independent end-to-end validation of missions is deliberately behind the interim volume target this week: the week went into making the validation itself trustworthy and into resolving and release-verifying the document-search issue, and the next step is to scale validation volume against the released build. Separately, an end-to-end solicitor-style workflow was run on the released build later in the day — document import by drag-and-drop, contract and Section 32 review, identification of pre-signing risks, and generation of a draft advice document — and it surfaced a new continuation issue at the email-preparation step. Gideon correctly stopped to ask for a client email address it could not verify; once that address was supplied it re-checked the matter instead of continuing in the one already open, and it stopped safely with nothing prepared, attached or sent. That issue is under investigation; the import, review and drafting steps worked, and no other blockers are open." },
    { date: "15 September 2026", text: "Gideon v0.1.35 is released and reaches the firm through the usual automatic update, with nothing to install by hand. It carries the reliability safeguards for running on a smaller local model. One local-model check — a full answer completing on the current test machine within the time limit — is still open and waits for more capable hardware." },
    { date: "14 September 2026", text: "The knowledge-base ingestion design was delivered to the firm today. It covers how imported material is sanitised before it can enter firm-wide knowledge, how each item is classified, how one matter stays walled off from another, and how an import runs from a clear caution and your confirmation through to a plain result showing what was added, what was kept matter-specific, and what needs your attention. It is being built in increments, with the confidentiality and matter-isolation rules as the foundation and the broader ingestion capability built on top, and each part tested before it ships; no date is set on the full pipeline yet. This is progress toward the knowledge and matter-files milestone, which remains upcoming. Separately, an on-device local-AI reliability issue the firm raised over the weekend was reproduced and traced to how the app handles slower on-device models. The fix gives a local model an appropriate window to respond, checks the model can handle the full request rather than letting part of it be dropped, and reports the real reason plainly if a local model cannot answer; the hosted path is unchanged. This is being verified on a real test machine before it is called done, and the item stays open until that proof is in. Validation work also moved forward: the validation foundation now reaches the live application, earlier pilot shortfalls were traced to a missing setup step in that foundation rather than to the product, the document path through the app was verified on the packaged application, and the setup design and a trial run are complete and queued as the priority route toward independently validating missions against the ratified pass standard. The conversational command layer (Milestone 2) continues toward its 18 September gate. No blockers are open." },
    { date: "11 September 2026", text: "The company-side GitHub billing item has cleared, so the dependency that was holding delivery of the next update is resolved. That update, v0.1.34, has now been released and verified. Gideon detects it automatically and downloads it in the background, and a restart moves the firm to the new version, with nothing to install by hand. The way each update is verified before delivery was strengthened, so every automated check must now pass on its own before an update can go out, and the assistant passes its full functional check with no failures, making each release more trustworthy. The conversational command layer (Milestone 2) continues toward its mid-September gate, and the knowledge-base ingestion design note stays on track for Monday 14 September." },
    { date: "10 September 2026", text: "Reliability and offline-AI safeguard work is built and verified in development, ready in the next update. When Gideon runs on a smaller offline model that is not reliable enough for a consequential action, it declines that action and recommends a capable model, and it asks for confirmation before an offline model is used on a matter, so the firm stays in control of when local-only use happens. Gideon can now prepare an email with a document it has already drafted attached, held for approval, with nothing sent until it is approved and what goes out being exactly what was approved; this was proven end to end, with the correct document arriving intact at the recipient. It also stays on the matter in hand, treating a person's name as the recipient rather than another matter to open, while genuine cross-matter references still require explicit confirmation. None of this is released yet: the update carrying it is built and its delivery is the only pending step, held by a company-side GitHub billing item now with the account holder. Engineering and validation continue as normal, and the conversational command layer and the Monday knowledge-ingestion design note continue toward their gates." },
  ...
  dailyLog: [
    { date: "18 September 2026", items: [
      "Released Gideon v0.1.37 and verified it on the actual released Windows build through the automatic update path: Gideon detected the new version, downloaded it, restarted into v0.1.37, and the document-search fix was re-tested on the updated installation. A natural-language search now reliably finds the relevant matter documents where the earlier build could miss them, and keeps the full set in its response.",
      "Added drag-and-drop into the matter conversation: dropping a file into the conversation opens the existing check-before-importing review bound to that matter, and nothing imports until you confirm. Confirmed on the packaged Windows build rather than only in development.",
      "Completed the Milestone 2 matter-recognition validation review. Gideon works within the matter you have open, protects against acting in the wrong matter, and asks for clarification rather than guessing when a reference is ambiguous or cannot be safely identified. Engineering validation has passed; the remaining acceptance step is the firm's confirmation that this meets expectations in practice, so Milestone 2 stays in progress. Carrying standing instructions forward automatically remains separate follow-on work, not part of this acceptance gate.",
      "Independent end-to-end validation of missions is deliberately behind this week's interim volume target: the week went into making the validation itself trustworthy and into resolving and release-verifying the document-search issue rather than producing a fast number. Official validated missions remain at zero and stay separate from diagnostic checks. The next step is to scale validation volume against the released build.",
      "An end-to-end solicitor-style workflow on the released build — drag-and-drop import, contract and Section 32 review, pre-signing risk identification, and a draft advice document — surfaced a new continuation issue at the email step: after Gideon correctly stopped to ask for a client email address it could not verify, supplying that address made it re-check the matter instead of continuing in the one already open. It stopped safely, with nothing prepared, attached or sent, and the issue is under investigation.",
      "The conversational command layer (Milestone 2) continues toward acceptance and Milestone 3 continues in parallel. No other blockers are open."
    ] },
    ...
    { date: "14 September 2026", items: [
      "Delivered the knowledge-base ingestion design to the firm: imported material is sanitised before it can enter firm-wide knowledge, each item is classified, matters stay walled off from one another, and an import runs from a clear caution and your confirmation through to a plain result showing what was added to firm knowledge, what was kept matter-specific, and what needs attention. It is being built in increments, confidentiality and matter-isolation first and the broader ingestion capability on top, with each part tested before it ships and no date set on the full pipeline yet. This is progress toward the knowledge and matter-files milestone, which stays upcoming.",
      "Reproduced an on-device local-AI reliability issue the firm raised over the weekend and traced it to how the app handles slower on-device models. Built a fix that gives a local model an appropriate window to respond, checks it can handle the full request rather than letting part of it be dropped, and reports the real reason plainly when a local model cannot answer; the hosted path is unchanged. The fix is being verified on a real test machine and stays open until that proof is in, with nothing called done before then.",
      "Moved the validation work forward: the validation foundation now reaches the live application, earlier pilot shortfalls were traced to a missing setup step in that foundation rather than to the product, the document path through the app was verified on the packaged application, and the setup design and a trial run are complete and queued as the priority route toward independently validating missions against the ratified pass standard.",
      "The conversational command layer (Milestone 2) continues toward its 18 September gate. No blockers are open."
    ] },
    { date: "11 September 2026", items: [
      "The company-side GitHub billing item cleared, resolving the dependency that was holding delivery of the next update. That update, v0.1.34, has now been released and verified. It reaches the firm through the usual automatic update, with nothing to install by hand.",
      "Strengthened how each update is verified before delivery. Every automated check must now pass on its own before an update can go out, and the assistant passes its full functional check with no failures, so each release is more trustworthy.",
      "Advanced the conversational command layer (Milestone 2), which continues toward its mid-September gate and is being validated against real-world use, with recognising the intended matter kept deliberately separate from acting on it so a cross-matter instruction still requires explicit confirmation.",
      "The knowledge-base ingestion design for the 2 October gate stays on track for the design note on Monday 14 September."
    ] },
    { date: "10 September 2026", items: [
      "Built and verified safeguards for running Gideon on a smaller offline model. When such a model is not reliable enough for a consequential action, Gideon declines that action and points to a capable model rather than proceeding, and it asks for confirmation before an offline model is used on a matter, so control over local-only use stays with the firm. These are verified in development and held for the next update, not yet released or seen on the device.",
      "Made preparing an email with an already-drafted document attached reliable and gated on approval, and proved it end to end: an existing drafted document was attached, nothing was sent until it was approved, and the correct document arrived intact at the recipient. Verified on the running build in development and held for the next update, not yet on the installed device.",
      "Corrected the handling so an instruction about the matter in hand is acted on there and a person's name is treated as the recipient rather than mistaken for another matter, while genuine cross-matter references still require explicit confirmation. Verified in the same end-to-end run and held for the next update.",
      "The next update carrying this work is built and ready. Its delivery to the firm is the only pending step and remains held by a company-side GitHub billing item, now with the account holder. Engineering and validation continue as normal.",
      "Advanced the knowledge-base ingestion design for the 2 October gate, on track for the design note on Monday 14 September."
    ] },
