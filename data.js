window.DASHBOARD_DATA = {
  ...
  kpis: [
    ...
    { key: "blockers", label: "Blockers", value: "0", subLabel: "None open", detail: "The release dependency that was open has cleared: the company-side GitHub billing item is resolved. The next update is not yet released. It is being held deliberately for full verification before delivery, so it ships only once the complete verification path passes rather than on partial checks." },
  ...
  remarks: [
    { date: "11 September 2026", text: "The company-side GitHub billing item has cleared, so the dependency that was holding delivery of the next update is resolved. That update is not being rushed out: it is held deliberately for full verification before it reaches the firm, and is released only once the complete verification path passes rather than on partial checks. The way each update is verified before delivery was strengthened, so every automated check must now pass on its own before an update can go out, and the assistant passes its full functional check with no failures, making each release more trustworthy. The conversational command layer (Milestone 2) continues toward its mid-September gate, and the knowledge-base ingestion design note stays on track for Monday 14 September." },
  ...
  dailyLog: [
    { date: "11 September 2026", items: [
      "The company-side GitHub billing item cleared, resolving the dependency that was holding delivery of the next update. That update is built and is now held deliberately for full verification before it is delivered, rather than released on partial checks. It is not yet released.",
